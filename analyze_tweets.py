...
# print(df.columns)
# df1 = df[['created_at', 'entities','id','text','user']]
...
def main():
    ...
    tweets = api.get_tweets(query = 'malaysia', count = 200)
...
def search_word(keyword):
    ckey = "2lxsiNRLXLws2wTfrgA8FbDfm"
    csecret = "NWxZi50qaggTDi57iDC5bAHGtw6wxVudgJFUaYsuD0nZH0rBDj"
    atoken = "991597159031103488-xTpAXHNJonWPbG5Xk0xyusXI0X7CIoo"
    asecret = "l6iVc9kOR6UuuroSfeCmx14CYjbTTqFoGGtZdb96xRlSZ"
    
    OAUTH_KEYS = {'consumer_key':ckey, 'consumer_secret':csecret,'access_token_key':atoken, 'access_token_secret':asecret}
    auth = tweepy.OAuthHandler(OAUTH_KEYS['consumer_key'], OAUTH_KEYS['consumer_secret'])
    api = tweepy.API(auth)

    tweets = tweepy.Cursor(api.search, q='unifi').items(20)
    for tweet in tweets:
        print('----')
        print(tweet.geo)
        print(tweet.text)
